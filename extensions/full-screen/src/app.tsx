...
import { Connection } from "../deps/beautiful-lyrics/Packages/Signal"
...
async function main() {
    ...
    function render() {
        ...
        if (!CFM.get("lyricsDisplay") || !CFM.get("extraControls")) container.classList.remove("lyrics-hide-force")
    ...
    function updateUpNextShow() {
        setTimeout(() => {
            const timetogo = Utils.getShowTime(CFM.get("upnextTimeToShow") as Settings["upnextTimeToShow"])
            if (upnextTimer) {
    ...
    async function updateInfo() {
        ...
        coverImg.onload = () => {
            ...
            if (CFM.get("lyricsDisplay") && CFM.get("autoHideLyrics")) {
                const lyricsContainer = container.querySelector<HTMLElement>("#fad-lyrics-plus-container")
                if (lyricsContainer) {
        ...
        switch (CFM.get("invertColors")) {
            ...
            case "auto": {
                let mainColor = "255,255,255",
                    contrastColor = "0,0,0"
                if (CFM.get("backgroundChoice") === "album_art" && meta.album_uri.split(":")[2] in INVERTED) {
    ...
    async function updateThemeColor(imageURL: string) {
        if (
            !(CFM.get("backgroundChoice") == "dynamic_color" && CFM.get("coloredBackChoice") == "VIBRANT") &&
            ...
            const artColors = await WebAPI.colorExtractor(imageURL).catch(err => console.warn(err))
    ...
    function autoHideLyrics() {
        const lyricsContainer = container.querySelector("#fad-lyrics-plus-container") as HTMLElement
    ...
    async function updateContext() {
        const ctxDetails = await Utils.getContext(translations[LOCALE]).catch(err => console.error(err))
        ...
        ctx_icon.innerHTML = /^<path/.test(ctxDetails!.ctxIcon)
            ? `<svg width="48" height="48" viewBox="0 0 16 16" fill="currentColor">${ctxDetails!.ctxIcon}</svg>`
            : ctxDetails!.ctxIcon
    ...
    function updateHeart() {
        ...
        if (prevHeartData !== meta?.metadata["collection.in_collection"]) Utils.fadeAnimation(heart)
    ...
    function toggleInvert() {
        ...
        if (invertButton.classList.contains("button-active")) invertButton.innerHTML = ICONS.INVERT_ACTIVE
    ...
    async function activate() {
        ...
        if (CFM.get("progressBarDisplay")) {
            ReactDOM.render(<SeekableProgressBar />, container.querySelector("#fsd-progress-parent"))
        }
        if (CFM.get("playerControls")) {
    ...
    function openConfig(evt: Event | null = null): void {
        ...
        configContainer.append(
            ...
            createToggle(translations[LOCALE].settings.progressBar, "progressBarDisplay", value => {
                CFM.set("progressBarDisplay", value)
                if (value) ReactDOM.render(<SeekableProgressBar />, container.querySelector("#fsd-progress-parent"))
                else ReactDOM.unmountComponentAtNode(container.querySelector("#fsd-progress-parent")!)
            }),
            ...
            createInputElement(translations[LOCALE].settings.staticColor, "staticBackChoice", "color", value => {
    ...
    let lastConnection: Connection<(Song?: Song | undefined) => void> | undefined
    trapElement("#BeautifulLyrics", lyricsContainer => {
        lastConnection?.disconnect()

        if (!lyricsContainer) return

        lyricsContainer.querySelector("div.RootContainer")?.remove()
        ...
        lastConnection = Player.SongChanged.connect(HandlePageView)
        HandlePageView()
    })
}
