...
export function animatedRotatedCanvas(back: HTMLCanvasElement, bgImg: HTMLImageElement) {
    ...
    const brightness = Math.min(CFM.get("backgroundBrightness") as Settings["backgroundBrightness"], 0.7)
...
function getSizeValues(parentWidth: number, parentHeight: number, childWidth: number, childHeight: number) {
