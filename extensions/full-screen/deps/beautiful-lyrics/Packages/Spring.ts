...
class Spring {
    ...
    public Update(deltaTime: number): number {
        ...
        if (this.DampingRatio == 1) {
            newPosition =
                (offset * (1 + radialFrequency * deltaTime) + velocity * deltaTime) * decay + final;
            newVelocity =
                (velocity * (1 - radialFrequency * deltaTime) -
                    offset * (radialFrequency * radialFrequency * deltaTime)) *
                decay;
            ...
            newVelocity =
                (velocity * (i - z * dampingRatio) - offset * (z * radialFrequency)) * decay;
        } else {
            // Overdamped
