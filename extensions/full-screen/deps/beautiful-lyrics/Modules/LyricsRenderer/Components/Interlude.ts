...
type AnimatedDot = {
    // Time is relative to the Syllable
    Start: number
    Duration: number
    GlowDuration: number
...
// Class
export default class InterludeVisual implements SyncedVocals, Disposable {
    // Private Properties
    ...
    public constructor(lineContainer: HTMLElement, interludeMetadata: Interlude) {
        ...
        {
            // Clone our ranges
            const scaleRange = MainAnimations.BaseScaleRange.map(point => {
                return {
                    Time: point.Time,
                    Value: point.Value,
                }
            })
            const opacityRange = MainAnimations.OpacityRange.map(point => {
                return {
                    Time: point.Time,
                    Value: point.Value,
                }
            ...
            this.ScaleSpline = new CurveInterpolator(scaleRange.map(metadata => [metadata.Time, metadata.Value]))
            this.OpacitySpline = new CurveInterpolator(opacityRange.map(metadata => [metadata.Time, metadata.Value]))
        }
    ...
    public Animate(songTimestamp: number, deltaTime: number, isImmediate?: true) {
        ...
        const pastStart = relativeTime >= 0,
            beforeEnd = relativeTime <= this.Duration
        ...
        if (stateChanged) {
            ...
            if (this.State !== "Sung") {
                // We evaluate this once we've stabilized our animation
                this.EvaluateClassState()
            }
        ...
        if (shouldUpdateVisualState || isMoving) {
            ...
            for (const dot of this.Dots) {
                ...
                {
                    if (shouldUpdateVisualState) {
                        this.UpdateLiveDotState(dot.LiveText, dotTimeScale, dotTimeScale, isImmediate)
                    }

                    if (isMoving) {
                        const dotIsSleeping = this.UpdateLiveDotVisuals(dot.LiveText, deltaTime)
