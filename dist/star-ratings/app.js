...
// shared/fp.ts
import { array as ar, eq, string as str, record as rec, semigroup as sg, function as f } from "https://esm.sh/fp-ts"
...
var waitForElement = (selector, timeout = 1e3, location = document.body, notEl) =>
    new Promise(resolve => {
        const res = v => {
            observer.disconnect()
            resolve(v)
...
var toggleLiked = async uris => {
    ...
    return await f2.pipe(
        uris,
        a.reduceWithIndex([[], []], (i, acc, uri) => (acc[Number(liked[i])].push(uri), acc)),
        ([toAdd, toRem]) => {
            const ps = []
...
var removeWebPlaylistTracks = async (playlist, tracks) =>
...
var fetchPlatArtistLikedTracks = async (uri, offset = 0, limit = 100) =>
...
var createPlatPlaylist = async (name, location = {}) =>
    await Spicetify.Platform.RootlistAPI.createPlaylist(name, location)
var setPlatPlaylistVisibility = async (playlist, visibleForAll) =>
...
var addPlatPlaylistTracks = async (playlist, tracks, location = {}) =>
...
var findModuleByStrings = (modules2, ...filters) =>
    modules2.find(f7 =>
        allPass(filters.map(filter => (typeof filter === "string" ? s => s.includes(filter) : s => filter.test(s))))(
            f7.toString(),
        ),
    )
...
var SettingsSection = class _SettingsSection {
    constructor(name, id, sectionFields = {}) {
        ...
        this.SettingsSection = () =>
            /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement(SectionTitle, null, this.name),
        ...
        this.SettingField = ({ field, children }) =>
            /* @__PURE__ */ React.createElement(
                SettingColumn,
                ...
                /* @__PURE__ */ React.createElement(
                    ...
                    /* @__PURE__ */ React.createElement(SettingText, { htmlFor: field.id }, field.desc),
                ),
                /* @__PURE__ */ React.createElement("div", { className: "x-settings-secondColumn" }, children),
            )
        this.ButtonField = field =>
            /* @__PURE__ */ React.createElement(
                ...
                /* @__PURE__ */ React.createElement(
                    ButtonSecondary,
                    { id: field.id, buttonSize: "sm", onClick: field.onClick, className: "x-settings-button" },
                    field.text,
                ),
            )
    ...
    static {
        this.setDefaultFieldValue = async (id, defaultValue) => {
            if (_SettingsSection.getFieldValue(id) === null) _SettingsSection.setFieldValue(id, await defaultValue())
        }
    }
}
...
var setStarsGradientFromContainerByRating = rating =>
    f4.flow(getStarsStopsFromStarsContainer, setStarsGradientByRating(rating))
...
var getTrackListTrackUri = track => (
    (track = Object.values(track)[0].child.child.child.child), track.pendingProps.uri ?? track.child.pendingProps.uri
)

// extensions/star-ratings/settings.ts
var settings = new SettingsSection("Star Ratings", "star-ratings")
    ...
    .addInput({ id: "heartThreshold", desc: "Threshold for liking trakcs", inputType: "text" }, task2.of("3.5"))
    .addInput({ id: "skipThreshold", desc: "Threshold for skipping trakcs", inputType: "text" }, task2.of("1.5"))
    .addInput(
        {
            id: "ratingsFolderUri",
...
var aggregateRatings = uris =>
    f5.pipe(
        ...
        readonlyArray.unzip,
        ([rs, wrs]) =>
            readonlyArray.reduceWithIndex(0, (i, s, r) => s + r * wrs[i])(rs) /
                readonlyArray.reduce(0, (s, wr) => s + wr)(wrs) || 0,
    )
...
var loadRatings = async () => {
    ...
    playlistUris = f5.pipe(
        ...
        ar3.reduce([], (acc, [type, uri, starsN]) => (type === "playlist" && starsN ? (acc[starsN] = uri) : [], acc)),
    )
    globalThis.tracksRatings = tracksRatings = await f5.pipe(
        ...
        pMchain(
            ar3.reduce({}, (acc, [trackUri, rating]) =>
                Object.assign(acc, {
...
var updateTrackListStars = f6.flow(
    ...
    ar4.map(trackList => {
        ...
        const firstElement = URI2.isArtist(locationUri)
            ? trackListTracks[0]
