...
var init_api = __esm(() => {
  ...
  searchYoutube = async (YouTubeApiKey, searchString) => (await (await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&q=${encodeURIComponent(searchString)}&type=video&key=${YouTubeApiKey}`)).json()).items;
});
