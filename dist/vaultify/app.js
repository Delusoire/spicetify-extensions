...
var React, ReactDOM, ButtonSecondary, require2, cache, modules, functionModules, SettingColumn, SettingText, SettingToggle, SettingsSection;
var init_settings = __esm({
  "shared/settings.tsx"() {
    ...
    require2 = webpackChunkopen.push([[Symbol()], {}, (re) => re]);
    cache = Object.keys(require2.m).map((id) => require2(id));
    modules = cache.filter((module) => typeof module === "object").map((module) => {
      try {
    ...
    SettingColumn = functionModules.find((m) => m.toString().includes("setSectionFilterMatchQueryValue"));
    SettingText = functionModules.find((m) => m.toString().includes("textSubdued") && m.toString().includes("viola"));
    SettingToggle = functionModules.find(
