...
var init_app = __esm(() => {
  ...
  extractLikedPlaylistTreeRecur = (leaf) => guard22([
    [
      isType("playlist"),
      async (playlist) => ({
        [playlist.name]: playlist.isOwnedBySelf ? await pipe(playlist.uri, fetchPlatPlaylistContents, pMchain(exports_Array.map((x) => x.uri))) : playlist.uri
      })
    ],
    [
      isType("folder"),
  ...
  backup = async () => {
    ...
    const settings3 = pipe(document.querySelectorAll(`[id^="settings."],[id^="desktop."],[class^="network."]`), Array.from, exports_Array.flatMap((setting) => {
