...
var vaultify = (() => {
  ...
  var equalsDefault, fromCompare, getSemigroup3, getMonoid3, strictOrd;
  var init_Ord = __esm({
    ".yarn/cache/fp-ts-npm-2.16.1-8deb3ec2d6-94e8bb1d03.zip/node_modules/fp-ts/es6/Ord.js"() {
      init_Eq();
  ...
  // shared/api.tsx
  var URI6, fetchWebArtistsSpot, fetchWebPlaylistsSpot, fetchWebAlbumsSpot, fetchWebTracksSpot, fetchPlatPlaylistContents, createPlatFolder, likePlatPlaylist, createSPPlaylistFromTracks, fetchPlatFolder, fetchPlatRootFolder, fetchTrackLFMAPI, fetchTrackLFMAPIMemoized;
  var init_api = __esm({
    "shared/api.tsx"() {
      "use strict";
      init_Function();
    ...
    "extensions/vaultify/app.tsx"() {
      ...
      extractLikedPlaylistTreeRecur = (leaf) => guard22([
        [
          isType("playlist"),
          async (playlist) => ({
            [playlist.name]: playlist.isOwnedBySelf ? await pipe(playlist.uri, fetchPlatPlaylistContents, pMchain(Array_exports.map((x) => x.uri))) : playlist.uri
          })
        ],
        [
          isType("folder"),
          async (folder) => ({
            [folder.name]: await pipe(folder.items, map(extractLikedPlaylistTreeRecur), (ps) => Promise.all(ps))
          })
        ]
